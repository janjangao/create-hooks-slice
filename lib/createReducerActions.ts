import type {
  ...
  CaseDataReducers,
  DataState,
} from "./converter/reducerConverter.ts";
import reducerConverter, {
  getType,
  createAction,
  createReducer,
} from "./converter/reducerConverter.ts";
...
export type ActionHook<Payload = unknown> = (
  payload?: Payload
...
function setResourceStatusCaseReducer(
  state: DataStatusState<unknown>,
  action: PayloadAction<unknown>
) {
  Object.assign(state.status, action.payload);
}

export default function createReducerActions<
  Data = unknown,
...
>(
  ...
  const { actions, caseReducers } = reducerConverter<
    Data,
  ...
  const reducer = createReducer<DataStatusState<Data>, PayloadAction>(
    initialState,
    caseReducers
  );
