...
export type ThunkAction<Query, Result> = (
  query?: Query,
...
export type ThunkHook<TA extends ThunkAction<any, any>> = TA extends (
  ...
  ? <
    ...
    ) => HookQuery extends undefined
      ? (query: Query, thunkCallback?: ThunkCallback) => Promise<Result>
      : (query?: Query, thunkCallback?: ThunkCallback) => Promise<Result>
  : never;

export type ThunkHooks<TAS extends ThunkActions> = {
  [Key in keyof TAS as `useThunk${Capitalize<Key & string>}`]: ThunkHook<
    TAS[Key]
  >;
};
...
export default function createThunkHooks<
  TAS extends ThunkActions = ThunkActions
>(thunkActions: TAS) {
  const thunkHooks: { [k: string]: any } = {};
  Object.keys(thunkActions).forEach((actionName) => {
    ...
    const useThunkHook = (
      ...
      return (query: any, thunkCallback?: OptionalThunkCallback<any, any>) => {
        ...
        if (thunkCallback === undefined && hookThunkCallback !== undefined)
          thunkCallback = hookThunkCallback;
