...
type ThunkCallback<Payload, Error> = (
  payloadOrError: Payload | Error | undefined,
  dispatch: Dispatch
) => void;
...
export type OptionalThunkCallback<Payload, Error> =
  | ThunkCallback<Payload, Error>
  | PeriodThunkCallback<Payload, Error>;

export type ThunkAction<CQT extends CaseQueryThunk<any, any>> =
  CQT extends () => Promise<infer Result>
    ? (
        query?: undefined,
        thunkCallback?: OptionalThunkCallback<Result, any>
      ) => (dispatch: Dispatch) => Promise<Result>
    : CQT extends (query: infer Query) => Promise<infer Result>
    ? (
        query: Query,
        thunkCallback?: OptionalThunkCallback<Result, any>
      ) => (dispatch: Dispatch) => Promise<Result>
...
export type ThunkActions<CQTS extends CaseQueryThunks> = {
  [Key in keyof CQTS]: ThunkAction<CQTS[Key]>;
};

function normalizeThunkCallback<Payload, Error>(
  thunkCallback: OptionalThunkCallback<Payload, Error>
): PeriodThunkCallback<Payload, Error> {
...
function combineTwoThunkCallbacks<Payload, Error>(
  first?: ThunkCallback<Payload, Error>,
  second?: ThunkCallback<Payload, Error>
) {
  return first && second
    ? (payloadOrError: any, dispatch: Dispatch) => {
...
function combineThunkCallbacks<Payload, Error>(
  ...thunkCallbacks: OptionalThunkCallback<Payload, Error>[]
): PeriodThunkCallback<Payload, Error> {
  return (
    thunkCallbacks.length > 0
      ? thunkCallbacks.length === 1
        ? normalizeThunkCallback(thunkCallbacks[0])
        : thunkCallbacks.reduce((prev, next) => {
            ...
            const resultThunkCallback: PeriodThunkCallback<Payload, Error> = {
              ...
              ) as ThunkCallback<Payload, Error>,
...
function actionToThunkCallBack(
  action: ActionCreatorOptionalPayload
): ThunkCallback<any, any> {
  return (payloadOrError: any, dispatch: Dispatch) => {
    dispatch(action(payloadOrError));
  };
}

export default function createThunkActions<CQTS extends CaseQueryThunks>(
  thunks: CQTS,
  actions: ActionCreators = {}
) {
  const thunkActions: { [k: string]: any } = {};
  Object.keys(thunks).forEach((thunkName) => {
    ...
    thunkActions[thunkName] =
      (query?: any, thunkCallback?: OptionalThunkCallback<any, any>) =>
      async (dispatch: Dispatch) => {
        const thunkCallbacks = [actionThunkCallback, thunkCallback].filter(
          (v): v is OptionalThunkCallback<any, any> => !!v
        );
