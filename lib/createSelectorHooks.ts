...
export type CaseSelector<
  State,
  CDS extends CaseDataSelector<any, any>
> = CDS extends (data: any) => infer Result ? (state: State) => Result : never;
export type CaseSelectors<State, CDSS extends CaseDataSelectors<any>> = {
  [Key in keyof CDSS]: CaseSelector<State, CDSS[Key]>;
};
...
export type Selectors<
  Data,
  CDSS extends CaseDataSelectors<Data>
> = CoreSelectors<CaseSelectors<RootDataStatusState<Data>, CDSS>> &
  GetResourceStatusSelectors<RootDataStatusState<Data>>;

export type SelectorHook<SS extends AnySelector<any, any>> = SS extends (
  state: any
) => infer Result
  ? <SelectorTransformer = undefined>(
      selectorTransformer?: SelectorTransformer | ((result: Result) => any),
      deps?: any[]
    ) => SelectorTransformer extends undefined
      ? Result
      : SelectorTransformer extends (result: any) => infer TransformedResult
      ? TransformedResult
      : never
  : never;

export type SelectorHooks<SS extends AnySelectors<any>> = {
  [Key in keyof SS as `useSelector${Capitalize<Key & string>}`]: SelectorHook<
    SS[Key]
  >;
};
...
function makeSelector<State, Result>(
  selector: AnySelector<State, Result>,
  selectorTransformer: (data: any) => any
): AnySelector<State, Result> {
...
export function selectorConverter<
  Data = any,
  CDSS extends CaseDataSelectors<Data> = CaseDataSelectors<Data>
>(name: string, caseDataSelectors: CDSS) {
...
export function createSelectors<
  ...
  CDSS extends CaseDataSelectors<Data> = CaseDataSelectors<Data>
>(name: Name, caseDataSelectors: CDSS) {
  const selectors = coreCreateSelectors<RootDataStatusState<Data>>(
    selectorConverter<Data>(name, caseDataSelectors)
  );
  ...
  return { ...selectors, ...getResourceStatusSelectors } as Selectors<
    Data,
    CDSS
  >;
}
...
>(selectors: SS) {
  const selectorHooks: { [k: string]: any } = {};
