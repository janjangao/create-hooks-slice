...
export type ThunkHook<Query = unknown, Result = unknown> = (
  query: Query | null,
...
export function createThunkHooksFromThunkActions(thunkActions: ThunkActions) {
  ...
  Object.keys(thunkActions).forEach((actionName) => {
    ...
    thunkHooks[actionHookName] = (
      query: unknown,
      thunkCallback?: OptionalThunkCallback | unknown[],
      deps?: unknown[]
    ) => {
      ...
      return useMemo(
        () => thunkAction(query, finalThunkCallback)(dispatch),
        deps
      );
    };
  });
