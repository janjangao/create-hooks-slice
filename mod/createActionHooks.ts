...
import type { ActionCreatorOptionalPayload, ActionCreator, ActionCreators, PayloadAction } from "./core/createAction.ts";
...
export type ActionHook<AC extends ActionCreatorOptionalPayload<any>> = AC extends () => infer Result ? () => () => Result : AC extends (payload: infer Payload) => infer Result ? <HookPayload extends Payload | undefined>(hookPayload?: HookPayload) => HookPayload extends undefined ? (payload: Payload) => Result : (payload?: Payload) => Result : never;
export type ActionHooks<ACS extends ActionCreators> = { [Key in keyof ACS as `useAction${Capitalize<Key & string>}`]: ActionHook<ACS[Key]> };

function isFunction<S>(x: any): x is () => S {
  return typeof x === "function";
}

type SetResourceStatusReducers<Name> = { [Key in keyof any as `${Name & string}/_setResourceStatus`]: typeof setResourceStatusCaseReducer };
...
export function createReducerActions<Data = any, Name extends string = string, CDRS extends CaseDataReducers<Data> = CaseDataReducers<Data>>(name: Name, initialData: Data | (() => Data), reducers: CDRS) {
  ...
  const {
    actions,
    caseReducers
  } = createActions<Data, DataStatusState<Data>, Name, CDRS>(name, reducers);
  const setResourceStatusType = getType(name, setResourceStatusReducerName); // add internal setResourceStatus reducer

  const setResourceStatusReducers: SetResourceStatusReducers<Name> = {
    [setResourceStatusType]: setResourceStatusCaseReducer
  };
  const finalCaseReducers = { ...caseReducers,
  ...
  const reducer = createReducer<DataStatusState<Data>>(initialState, finalCaseReducers);
  return {
    name,
    reducer,
...
export default function createActionHooks<ACS extends ActionCreators = ActionCreators>(actions: ACS) {
  const actionHooks: {
    [k: string]: any;
  } = {};
