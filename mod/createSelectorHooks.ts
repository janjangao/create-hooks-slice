...
import type { CaseDataSelectors, RootState, Selector, Selectors } from "./converter/selectorConverter.ts";
import selectorConverter, { createSelectors as coreCreateSelectors, memoizeWithArgs } from "./converter/selectorConverter.ts";
export type { CaseDataSelectors, Selectors, Selector };
export type RootDataStatusState<Data = unknown> = RootState<DataStatusState<Data>>;
export type SelectorHook<Data = unknown, Result = unknown> = (selectorTransformer?: (data: Data) => Result, deps?: unknown[]) => Result;
...
function makeSelector<State>(selector: Selector<State>, selectorTransformer: (data: unknown) => unknown): Selector<State> {
...
export function createSelectors<Data = unknown, Name extends string = string>(name: Name, dataSelectors: CaseDataSelectors<Data>) {
  const selectors = coreCreateSelectors<DataStatusState, RootDataStatusState<Data>>(selectorConverter(name, dataSelectors)); // add internal getResourceStatus selector

  selectors[getResourceStatusSelectorName] = makeGetResourceStatusSelector(name);
  return selectors;
}
export function createSelectorHooksFromSelectors<State = unknown>(selectors: Selectors<State>) {
...
export default function createSelectorHooks<Data = unknown, Name extends string = string>(name: Name, dataSelectors: CaseDataSelectors<Data>) {
  return createSelectorHooksFromSelectors<RootDataStatusState<Data>>(createSelectors<Data, Name>(name, dataSelectors));
}