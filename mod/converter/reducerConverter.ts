...
import type { PrepareAction, PayloadAction, ActionCreator, ActionCreators } from "../core/createAction.ts";
...
export type CaseDataReducer<Data = unknown, Payload = unknown> = (data: Data | Draft<Data>, payload: Payload) => Data | void | Draft<Data>;
export type CaseDataReducerWithPrepare<Data, Payload = unknown> = {
  reducer: CaseDataReducer<Data>;
  prepare?: PrepareAction<Payload>;
};
export type SyncCaseDataReducer<Data = unknown, Payload = unknown> = CaseDataReducer<Data, Payload> | CaseDataReducerWithPrepare<Data, Payload>;
...
export type CaseDataReducers<Data = unknown, Payload = unknown> = {
  [key: string]: SyncCaseDataReducer<Data, Payload> | AsyncCaseDataReducer<Data, Payload>;
};
...
export type SyncCaseReducer<State, Payload = unknown> = CaseReducer<State, PayloadAction<Payload>> | CaseReducerWithPrepare<State, Payload>;

function convertToCaseReducer<Data, State extends DataState<Data> = DataState<Data>, Payload = unknown>(caseDataReducer: SyncCaseDataReducer<Data, Payload>): SyncCaseReducer<State, Payload> {
...
function extractCaseReducer<State>(maybeReducerWithPrepare: SyncCaseReducer<State>) {
...
export function createActions<Data = unknown, State extends DataState<Data> = DataState<Data>, Name extends string = string>(name: Name, reducers: CaseDataReducers<Data>) {
  if (!name) {
  ...
  reducerNames.forEach(reducerName => {
    ...
    if ("fulfilled" in maybeAsyncReducer) {
      Object.keys(maybeAsyncReducer).forEach(key => {
        ...
        if (asyncFullNameReducer) {
          const {
            caseReducer,
            prepareCallback
          } = extractCaseReducer<State>(convertToCaseReducer(asyncFullNameReducer));
      ...
      const {
        caseReducer,
        prepareCallback
      } = extractCaseReducer<State>(convertToCaseReducer(maybeAsyncReducer));
...
export default function converter<Data = unknown, State extends DataState<Data> = DataState<Data>, Name extends string = string>(name: Name, reducers: CaseDataReducers<Data>) {
