...
export type CaseQueryThunk<Query = unknown, Result = unknown> = (query?: Query) => Promise<Result>;
export type CaseQueryThunks<Query = unknown, Result = unknown> = {
  [key: string]: CaseQueryThunk<Query, Result>;
};
type ThunkCallback<Payload = unknown, Error = unknown> = (dispatch: Dispatch, payloadOrError?: Payload | Error) => void;
...
export type OptionalThunkCallback<Payload = unknown, Error = unknown> = ThunkCallback<Payload, Error> | PeriodThunkCallback<Payload, Error>;
export type ThunkAction<Query = unknown, Result = unknown> = (query: Query | null, thunkCallback?: OptionalThunkCallback) => (dispatch: Dispatch) => Promise<Result>;
export type ThunkActions<Query = unknown, Result = unknown> = {
  [key: string]: ThunkAction<Query, Result>;
};

function normalizeThunkCallback(thunkCallback: OptionalThunkCallback): PeriodThunkCallback {
...
function combineTwoThunkCallbacks(first?: ThunkCallback, second?: ThunkCallback) {
  return first && second ? (dispatch: Dispatch, payloadOrError?: unknown) => {
    first(dispatch, payloadOrError);
    second(dispatch, payloadOrError);
  } : first || second;
}

function combineThunkCallbacks(...thunkCallbacks: OptionalThunkCallback[]): PeriodThunkCallback {
  return ((thunkCallbacks.length > 0 ? thunkCallbacks.reduce((prev, next) => {
    ...
    const resultThunkCallback: PeriodThunkCallback = {
      ...
      fulfilled: (combineTwoThunkCallbacks(prevThunkCallback.fulfilled, nextThunkCallback.fulfilled) as ThunkCallback),
...
export default function createThunkActions(thunks: CaseQueryThunks, actions: ActionCreators = {}) {
  ...
  Object.keys(thunks).forEach(thunkName => {
    ...
    if (thunkName in actions || fulfilledActionName in actions) {
      ...
      const actionThunkCallback: OptionalThunkCallback = action ? actionToThunkCallBack(action) : {
    ...
    thunkActions[thunkName] = (query: unknown, thunkCallback?: OptionalThunkCallback) => async (dispatch: Dispatch) => {
      const actionThunkCallback = actionThunkCallbacks[thunkName];
      const thunkCallbacks = [actionThunkCallback, thunkCallback].filter((v): v is OptionalThunkCallback => !!v);
